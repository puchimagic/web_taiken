...
#    まとめて削除する
#
# このスクリプト自身は APP_BASE/project（プロジェクトフォルダ）の中にあるため、
# 4)の削除によりこのスクリプト自身も含めて丸ごと消える。デスクトップに配置している
# Webプロセットアップ.sh だけはこの削除対象に含まれないので、実行後デスクトップに
# 最終的に残るのは Webプロセットアップ.sh のみになる。何度でも
# Webプロセットアップ.sh -> Webプロクリーン.sh を繰り返せる（次にWebプロセットアップ.sh
# を実行すればプロジェクトフォルダとこのスクリプトも新しく作り直される）。
#
...
log ""
log "クリーンアップが完了しました。何かキーを押すと、このスクリプト自身を含むプロジェクトフォルダ一式を削除します（デスクトップのWebプロセットアップ.shは対象外です）。"
