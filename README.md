...
各PCのデスクトップに `setup-win.bat` を配置してダブルクリックすると、リポジトリのclone・VSCode拡張機能「PHP Server」の確認とインストール・スライドのデスクトップへのコピー・VSCodeでのフォルダオープン・PHPサーバーの起動までを一括で行います。
...
- エクスプローラーで `setup-win.bat` を右クリック→プロパティ→全般タブ下部の「許可する」にチェックを入れてOK
- または PowerShellで `Unblock-File "$env:USERPROFILE\Desktop\setup-win.bat"`（デスクトップがOneDriveと同期されている場合はパスを実際のデスクトップに読み替える）

サーバーへの接続が切れた場合など、起動だけをやり直したいときは、cloneされたフォルダ内の `start-win.bat` を直接実行してください（`setup-win.bat` の最後のステップと同じ処理です）。
