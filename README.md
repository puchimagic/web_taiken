...
各PCのデスクトップに `Webプロセットアップ.bat` を配置してダブルクリックすると、リポジトリのclone・VSCode拡張機能「PHP Server」の確認とインストール・スライドとクリーンアップ用bat(`Webプロクリーン.bat`)のデスクトップへのコピー・VSCodeでのフォルダオープン・PHPサーバーの起動までを一括で行います。
...
体験授業終了後、各PCを元の状態に戻したいときは、デスクトップの `Webプロクリーン.bat` を実行してください。PHPサーバーの停止・cloneしたプロジェクトフォルダ・スライド・ログの削除までを一括で行います（`Webプロセットアップ.bat` 自身とこのbat自身は削除しません）。
