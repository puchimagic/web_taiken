...
各PCのデスクトップに `Webプロセットアップ.bat` を配置してダブルクリックすると、リポジトリのclone（既にあれば削除して新しくclone。`.git`とREADMEは体験授業に不要なので取得後に削除）・VSCode拡張機能「PHP Server」の確認とインストール・スライドと`Webプロサーバー起動.bat`のデスクトップへのコピー・VSCodeでのフォルダオープン・PHPサーバーの起動までを一括で行います。実行するたびにまっさらな状態から始まるので、生徒が入れ替わるときも同じファイルを再実行するだけです。
